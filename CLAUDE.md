...
The panel ships with everything on: interval defaults to **Hour** and Answer Key, Color, Problem numbers, and Prevent duplicates are all `checked` in `index.html`. Those five settings (not the seed) are remembered locally: any change persists to `localStorage` under `clockSettings` via `saveSettings()`, and `applySavedSettings()` restores them on load. Precedence on load is defaults → saved settings → URL params → the initial worksheet, so a shared/bookmarked `?...` URL always wins over the locally remembered settings.
...
A single pencil (`#saved-tools`, left of the "Saved" eyebrow) toggles **edit mode** for the whole list. `enterSavedEdit()` clones the list into `savedDraft` and every edit mutates only the draft; the pencil becomes a single check that leaves edit mode and persists the draft (`commitSavedEdit()`). There is no discard/cancel button — leaving edit mode always saves. (`exitSavedEdit()` still exists as the shared teardown that clears `savedDraft`; `commitSavedEdit()` writes the draft first and then calls it.) In edit mode each chip gains three zones:
...
- Saved-worksheets **edit mode stages on a draft** — rename/delete/reorder mutate only `savedDraft`; nothing persists until you leave edit mode with the check (`commitSavedEdit()`), which then saves. There's no cancel button, so edits can't be abandoned once made.
